//! Laya's five presets from `laya/presets.py`, with the same question ids, instructions and
//! criteria, so answers from kime and Laya can be compared directly. Each is a `questions` map
//! for a request.

use serde_json::{Map, Value, json};

/// The preset names, in Laya's order.
pub const NAMES: [&str; 5] = ["triage", "email", "guard", "moderation", "router"];

/// The preset with this name, or `None`.
#[must_use]
...
/// Inbound email triage and threat filtering. `categories` replaces the six default teams, as a map from name to description.
#[must_use]
...
/// Real time guardrails for LLM input.
#[must_use]
pub fn guard() -> Value {
    json!({
        "jailbreak": {
...
/// Routing a request to the right language model.
#[must_use]
pub fn router() -> Value {
    json!({
        "difficulty": {
